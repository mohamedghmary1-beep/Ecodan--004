...
-- ---------------------------------------------------------------------
-- Some installations have already removed this legacy table. In that case,
-- skip this retirement step rather than failing the whole migration.
do $$
begin
...
-- Contractor requests can include source-file URLs and approval state. Scope
-- each row to its contractor and assigned engineer; administrators retain full
-- access for the approvals workflow.
drop policy if exists "contractor_requests_admin_only" on public.contractor_requests;
drop policy if exists "contractor_requests_select_participants" on public.contractor_requests;
drop policy if exists "contractor_requests_insert_contractor" on public.contractor_requests;
drop policy if exists "contractor_requests_update_engineer_or_admin" on public.contractor_requests;
